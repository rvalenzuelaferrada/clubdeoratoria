...
class ActionEvent extends Model
{
    ...
    public static function forResourceUpdate($user, $model)
    {
        return new static([
            ...
            'model_id' => $model->getKey(),
            'fields' => '',
            'changes' => $changes = array_diff_key($model->getDirty(), array_flip($model->getHidden())),
            'original' => array_intersect_key($model->newInstance()->forceFill($model->getRawOriginal())->attributesToArray(), $changes),
            'status' => 'finished',
            'exception' => '',
        ]);
    }
    ...
    public static function forAttachedResourceUpdate(NovaRequest $request, $parent, $pivot)
    {
        return new static([
            ...
            'model_id' => $pivot->getKey(),
            'fields' => '',
            'changes' => $changes = array_diff_key($pivot->getDirty(), array_flip($pivot->getHidden())),
            'original' => array_intersect_key($pivot->newInstance()->forceFill($pivot->getRawOriginal())->attributesToArray(), $changes),
            'status' => 'finished',
            'exception' => '',
        ]);
    }
